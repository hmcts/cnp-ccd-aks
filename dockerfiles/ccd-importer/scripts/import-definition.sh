...
userToken=$2
serviceToken=$(curl --silent -X POST http://service-auth-provider-api/testing-support/lease -d '{"microservice":"bulk_scan_orchestrator"}' -H 'content-type: application/json')

curl ${CURL_OPTS}  \
  http://ccd-definition-store-api/import \
